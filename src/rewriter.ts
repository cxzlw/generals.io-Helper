function rewriteGame(): void {
  ...
  for (let i = 1, cur: string, lastPos = -1; i < playerInfo.length; ++i) { 
     ...
     */
    confusingDiff[cur] = delta - Number(playerInfo[i].children[lboardCol - 2].textContent);
    ...
    if (lastPos !== -1) {    // 处理队伍数据
      playerInfo[lastPos].children[lboardCol - 2].textContent =
        (Number(playerInfo[lastPos].children[lboardCol - 2].textContent) + Number(playerInfo[i].children[lboardCol - 2].textContent)).toString();
      playerInfo[lastPos].children[lboardCol - 1].textContent =
        (Number(playerInfo[lastPos].children[lboardCol - 1].textContent) + Number(playerInfo[i].children[lboardCol - 1].textContent)).toString();
    }
  }
